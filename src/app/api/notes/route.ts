...
import { getEmbedding } from "@noteai/app/lib/openai";
import {
  createNoteSchema,
  deleteNoteSchema,
  updatedNoteSchema,
} from "@noteai/app/lib/vaildation/note";
...
export async function POST(req: Request) {
  try {
    ...
    if (!parseResult.success) {
      ...
      return Response.json({ erorr: "Invaild input" }, { status: 400 });
    }
...
export async function PUT(req: Request) {
  try {
    ...
    if (!note) {
      return Response.json({ error: "Not not found" }, { status: 404 });
    }
    const { userId } = auth();
